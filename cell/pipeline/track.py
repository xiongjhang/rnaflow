...
sys.path.append('/root/cell_track/bash/cell-tracker-gnn-main')
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if im_path is not None:
            flag = False
            im_num = im_path.split(".")[-2][-4:]

        if result_path is not None:
            flag = False
            result_num = result_path.split(".")[-2][-4:]
    ...
    def extract_freature_metric_learning(self, bbox, img, seg_mask, ind, normalize_type='MinMaxCell'):
        ...
        img.to(self.device)
        with torch.no_grad():
            embedded_img = self.embedder(self.trunk(img[None, None, ...].to(self.device)))
