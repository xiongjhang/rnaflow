...
from .utils import *
from .preprocess import hist_match, uint8_to_uint16
from .segment import load_detector, load_segmentor, uint16_to_uint8_maxmin, segment_fn, segment_postprocess_fn
from .track import create_csv, track_predict
from .postprocess import Postprocess, generate_track_csv, extract_track_res

os.environ['CUDA_VISIBLE_DEVICES'] = '0'

class CellPredicor(Predictor):
    """
    Cell Predictor class for handling cell segmentation and track tasks.
    """

    def __init__(self, stack_path: Path, dst_dir: Path):
        super().__init__()
        ...
        num_frames, height, width = self.raw_stack.shape
        del self.raw_stack
        ...
        self.raw_data_dir = self.dst_dir / '01'; self.raw_data_dir.mkdir(exist_ok=True)
        self.pre_data_dir = self.dst_dir / 'PRE' / 'PRE_MUL'; self.pre_data_dir.mkdir(exist_ok=True)
        self.gt_data_dir = self.dst_dir / '01_GT'; self.gt_data_dir.mkdir(exist_ok=True)
    ...
    def prepare(self):
        ...
        stack_to_frames(self.raw_stack, self.raw_data_dir)
    ...
    def segment(self):
        ...
        self.rgb_data_dir = self.dst_dir / 'PRE' / 'RGBGT'; self.rgb_data_dir.mkdir(exist_ok=True)
        ...
        self.seg_data_dir = self.gt_data_dir / 'SAMSEG'; self.seg_data_dir.mkdir(exist_ok=True)
        ...
        self.seg_post_data_dir = self.gt_data_dir / 'SEG'; self.seg_post_data_dir.mkdir(exist_ok=True)  # SEG_16
    ...
    def track(self):
        ...
        self.track_csv_dir = self.dst_dir / '01_CSV'; self.track_csv_dir.mkdir(exist_ok=True)
    ...
    def postprocess(self):
        ...
        self.track_seq_dir = self.dst_dir / '01_GT' / 'maskOriginal';  self.track_seq_dir.mkdir(exist_ok=True)
